...
def index(request):
	...
	for q in Quarter_Spending.objects.all():
		...
		supplies += q.supplies
		travel += q.travel
	return render(request, 'house_office_spending/table.html', context={'equipment': equipment, 'franked_mail': franked_mail, 'other_services': other_services, 'personnel': personnel, 'printing': printing, 'rent_utilities': rent_utilities, 'supplies': supplies, 'travel': travel, 'reps': House_Rep.objects.all()})
